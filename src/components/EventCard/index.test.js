  ...
  it("a title, a label and a month are displayed", () => {
    render(
      <EventCard
        ...
        title="test event"
        label="test label"
        date={new Date("2022-04-01")} // Date en avril
      />
    );
    const titleElement = screen.getByText(/test event/);
    const monthElement = screen.getByText(/avril/); // Le mois d'avril est attendu
    const labelElement = screen.getByText(/test label/);
    
    expect(titleElement).toBeInTheDocument();
    expect(labelElement).toBeInTheDocument();
    expect(monthElement).toBeInTheDocument(); // Vérification que le mois "avril" est affiché
  });
